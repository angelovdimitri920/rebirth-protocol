...
**Before doing any work, read `docs/00_CANON_INDEX.md` and `docs/GAME_DESIGN.md` in full.** The canon index is the creative-documentation authority map: for lore, theme, naming, factions, characters, story, or high-level gameplay work, read the relevant numbered document and check `docs/09_CANON_LEDGER_AND_BACKLOG.md` before changing a creative fact. `GAME_DESIGN.md` remains the detailed mechanics, implementation-history, and staged-roadmap reference, while `docs/08_GAMEPLAY_TOURNAMENT_AND_PROGRESSION.md` owns the high-level gameplay contract. `docs/SETTING_AND_FACTIONS.md`, `docs/WORLD_HISTORY.md`, and `docs/RELIGION_AND_IDEOLOGY.md` remain detailed supporting references under that hierarchy. They and the numbered documents supersede `WARBAND_THEME_REFERENCE.md`'s old literal-culture framing. **Edictbound** is the only canonical term for the forbidden technological condition. Do not invent mechanics that contradict the applicable authority; if a design decision is genuinely ambiguous or missing, make a reasonable call, note it, and keep moving rather than stalling on small things — but flag anything with a real gameplay-feel tradeoff.
