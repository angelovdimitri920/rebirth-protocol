...
**Before doing any work, read `docs/GAME_DESIGN.md` in full**, plus `docs/SETTING_AND_FACTIONS.md` (the canonical world/theme — supersedes `WARBAND_THEME_REFERENCE.md`). `GAME_DESIGN.md` is the source of truth for mechanics and the staged roadmap. Do not invent mechanics that contradict it; if a design decision is genuinely ambiguous or missing, make a reasonable call, note it, and keep moving rather than stalling on small things — but flag anything with a real gameplay-feel tradeoff.
