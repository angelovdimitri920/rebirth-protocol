...
**World canon (2026-07-21, PR #19)**: `docs/WORLD_HISTORY.md` (backstory/chronology) and `docs/RELIGION_AND_IDEOLOGY.md` (5 institutions + subfactions) joined `SETTING_AND_FACTIONS.md` as the canonical world-canon set — **Edictbound** is the only correct name for the banned tier (never "Hushforged"). Every part still just needs a working mechanic + a reasonable blurb — lore fidelity is a nice-to-have, not a blocker — but the following rows touch faction/religious content directly enough to be worth a quick cross-reference before writing blurbs: **F** (Fetterlock/Knell Maul are Umbral Concordat-stewarded, Rime Charge/Hoarfrost Ward/Winterwatch are Order of the Winter Wing-stewarded — `ARMORY_REFERENCE.md` §15), **M** (each new chassis needs its Order confirmed against `SETTING_AND_FACTIONS.md`'s faction table), **P** (the Edictbound tier now has expanded rules in `ARMORY_REFERENCE.md` §16 — dormancy, cumulative risk *flagged as a future/unimplemented mechanic*, pilot danger, and faction-distinct treatment: Whole Crown masters it secretly, Broken Choir joins it openly, Ash Levelers weaponize-then-destroy it), **O** (laurels/Class ranks already align; the Casting ritual may want Communion-investiture flavor), **R** (the Lasthold hub is where all of `RELIGION_AND_IDEOLOGY.md` actually gets used — shrines, NPC dialogue, faction reputation).
...
| **F** | ~~Fetter status~~ **done** — [PR #21](https://github.com/angelovdimitri920/rebirth-protocol/pull/21) | ~~Immobilize distinct from knockdown (+2 s fetter-immunity rule) → **Fetterlock, Rime Charge, Winterwatch, Knell Maul, Tocsin Mace, Hoarfrost Ward**~~ | ARMORY; DOCTRINE §13 pillar 9 |
| **G** | ~~Pulls & piercing~~ **done** — [PR #22](https://github.com/angelovdimitri920/rebirth-protocol/pull/22) | ~~On-hit pull forces; guard-piercing → **Grapnel, Hookbill, Estoc, Auger, Sawtooth Espadon**~~ | ARMORY |
| **H** | ~~Scaling & delayed threats~~ **done** — [PR #23](https://github.com/angelovdimitri920/rebirth-protocol/pull/23) | ~~Distance/lunge/speed damage scaling; hanging/delayed actives → **Pilgrim, Tilt Lance, Courser Saber, Vigil, Penitent Flail, Beacon, Crowbeak Pick**~~ | ARMORY §13.1 |
| **I1** | ~~Trajectory guns + casting wave~~ **done** — [PR #24](https://github.com/angelovdimitri920/rebirth-protocol/pull/24) | ~~Arcing/vaulting/dropping/looping gun paths + the first projectile-casting melee, all `ProjectileSystem` → **Mangonel, Evenfall, Skysword, Falconet, Volant Falx**~~ | ARMORY |
| **I2** | ~~Trajectory bombs~~ **done** — [PR #25](https://github.com/angelovdimitri920/rebirth-protocol/pull/25) | ~~The `BombSystem` half of Task I: extreme vertical arc, curving trajectory + contact detonation, dwelling mines → **Steeplefall, Oxbow Charge (D/S), Oubliette Mine, Oubliette Twin**~~ | ARMORY §6 |
