...
`RoboMesh.ts` originally built every chassis from a handful of plain `THREE.BoxGeometry` slabs — functional for silhouette-testing, but flat and un-articulated. All four have since gotten a real Gundam/Armored-Core panel-language detail pass (`GAME_DESIGN.md` §14-15), informed by four actual reference model rips the user provided (Ray Warrior→Legionnaire, Metal Ox→Crusader Knight, Seeker→Valkyrie, Shrike→Shinobi) — used only for extracting proportion/color data locally, never imported or embedded (§1's "no copy of a named design" rule held for actual assets too, not just web references).
