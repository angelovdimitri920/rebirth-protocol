...
## 5. Mesh redesign direction

`RoboMesh.ts` currently builds every chassis from a handful of plain `THREE.BoxGeometry` slabs — functional for silhouette-testing, but flat and un-articulated. The redesign target per chassis, applying Gundam/Armored-Core panel language on top of each civilization's real silhouette:

- **Legionnaire (Vanguard)**: layered torso plating suggesting *lorica segmentata*'s banded strips rather than one smooth box; a rectangular shoulder/chest motif echoing a scutum shield; a more helmet-like head silhouette (browline + crest ridge) instead of a plain box head.
- **Knight (Bulwark)**: taller, harder shoulder/pauldron silhouette; a visor-slit "great helm" head; heavier segmented limb plating to sell the bulk honestly rather than just scaling up box sizes.
- **Valkyrie (Skylance)**: wing-blade elements at the back/shoulders (structural, not literal feathers) that read as flight-coded without being a literal bird; a sharper, more angular chest/head silhouette than the current rounded box.
- **Shinobi (Wraith)**: a low hood/cowl head silhouette instead of a boxy helmet; a more angular, minimal-surface-area torso; the existing "cape/vent panel" becomes a proper trailing cloth-coded silhouette element.

This is real mesh code work, done incrementally — one chassis fully redesigned and visually verified before moving to the next, per the project's standing "small testable increments" rule (`CLAUDE.md`).
